...
fdt.addEvent = function(el, type, handler) {
  if (el.attachEvent) el.attachEvent('on'+type, handler); else el.addEventListener(type, handler);
}
fdt.removeEvent = function(el, type, handler) {
  if (el.detachEvent) el.detachEvent('on'+type, handler); else el.removeEventListener(type, handler);
}
fdt.onReady = function(ready) {
...
// Find element that is currently pointing to the passed url by css class
  fdt.findElementByClassUrl = function (cssclass, url) {
...
fdt.openMenus = function (menuElements) {
...
function initNav() {
  const menuElements = fdt.findElementByClassUrl("menu-link", window.location.href);
...
function searchLoaded(index, docs) {
  ...
  function update() {
    ...
    function addResult(resultsList, result) {
      ...
      resultDoc.innerHTML = '<svg height="16" class="octicon octicon-file" viewBox="0 0 12 16" version="1.1" width="12" aria-hidden="true"><path fill-rule="evenodd" d="M6 5H2V4h4v1zM2 8h7V7H2v1zm0 2h7V9H2v1zm0 2h7v-1H2v1zm10-7.5V14c0 .55-.45 1-1 1H1c-.55 0-1-.45-1-1V2c0-.55.45-1 1-1h7.5L12 4.5zM11 5L8 2H1v12h10V5z"></path></svg>';
  ...
  fdt.addEvent(searchInput, 'focus', function(){
    setTimeout(update, 0);
  });

  fdt.addEvent(searchInput, 'keyup', function(e){
    switch (e.keyCode) {
  ...
  fdt.addEvent(searchInput, 'keydown', function(e){
    switch (e.keyCode) {
      case 38: // arrow up
        e.preventDefault();
  ...
  fdt.addEvent(document, 'click', function(e){
